...
class SearchForWeatherRepository(private val searchForWeatherService: ISearchForWeatherService) :
    ISearchForWeatherRepository {
    override suspend fun searchForWeather(query: String): Flow<List<CitiesEntity>?> = flow {
      val response = searchForWeatherService.searchForWeather(query).body<List<SearchForWeatherResponseItem>?>()
